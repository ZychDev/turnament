...
export async function readDb() {
  ...
  if (!result.rows?.length) throw new Error('Tournament data not found');
  try { return JSON.parse(result.rows[0].data); } catch { throw new Error('Corrupted tournament data'); }
}
...
export async function readHistory() {
  ...
  return result.rows.map(r => {
    try { return { timestamp: r.timestamp, state: JSON.parse(r.state) }; }
    catch { return null; }
  }).filter(Boolean);
}
...
export async function undoLast() {
  ...
  let state;
  try { state = JSON.parse(last.state); } catch { return null; }
...
export async function readPostComments(postId) {
  ...
  return result.rows.map(r => ({ id: Number(r.id), postId: Number(r.postId), nickname: r.nickname, message: r.message, timestamp: Number(r.timestamp) }));
}
