...
export async function getPlayerProfile(gameName, tagLine) {
  ...
  // Recent match IDs — no type filter to include ALL game types (ranked, normal, custom, aram)
  // Note: Riot API valid types are: ranked, normal, tourney, tutorial
  // Custom games (queueId=0) appear only without type filter
  let recentMatchIds = [];
  try {
    recentMatchIds = await getMatchIds(account.puuid, 20, null) || [];
  } catch (e) { /* ignore */ }

  // Fetch match details for recent games
  const recentMatches = [];
  const champStats = {}; // track winrate per champion
  for (const id of recentMatchIds.slice(0, 15)) {
    try {
  ...
  const championWinrates = Object.entries(champStats)
    .map(([champ, s]) => ({
      ...
      games: s.games,
      wins: s.wins,
      winRate: s.games > 0 ? ((s.wins / s.games) * 100).toFixed(0) : '0',
  ...
  return {
    ...
    flexQ: flexQ ? {
      ...
      wins: flexQ.wins,
      losses: flexQ.losses,
      winRate: (flexQ.wins + flexQ.losses) > 0 ? ((flexQ.wins / (flexQ.wins + flexQ.losses)) * 100).toFixed(1) : '0',
    } : null,
    topChampions,
    championWinrates,
    recentMatches,
